...
The NYPD shooting data covers shootings in the five boroughs of NY, that may or may not have resulted in death of the victim. Geographical location; age, gender, and race info on the perpetrator and victim; time and date of occurrence 
...
```{r}
df <- select(df, - c(JURISDICTION_CODE,PRECINCT,LOCATION_DESC,PERP_RACE,VIC_RACE, Latitude:Lon_Lat))
head(df)
```
## Summarize data set
...
```{r}
coord_xy_2 <- ggplot(df, aes(x = X_COORD_CD, y = Y_COORD_CD, color = as.factor(BORO))) +
  geom_point()
...
```{r}
vic_age <- ggplot(df, aes(x = VIC_AGE_GROUP, fill = factor(VIC_AGE_GROUP))) +
  geom_bar()
...
```{r}
shootings_hour<- ggplot(df, aes(hour, fill=as.factor(MURDER))) +
  geom_bar()
...
Number if shootings in different days of the week (another student's code).
...
In general, there were more shooting cases in summer months (another student's code).
...
df_m %>% ggplot(aes(x = MONTH, y = n, color = YEAR, group = YEAR)) +
  ...
  geom_line(size = 1) +
  labs(title = "NYC shooting incidents by month",
  x = "Month", y = "Count of incidents per month")
...
Count the number of shootings occurring each day.

```{r}
df_day <- df %>%
  mutate(DAY=factor(OCCUR_DATE)) %>%
  group_by(DAY) %>%
  count() %>%
...
Shootings timeline  --- needs more work ????

```{r}

#df_20 <- df_day[which(df_day$DAY >= 2020-01-01), ]

df_day <- df_day %>% 
  ggplot(aes(x = DAY, y = n)) +
  geom_point(size = 1) +
  labs(title = "NYC shooting incidents",
  x = "Dates", y = "Count of incidents")
...
```{r}
mod_1 = lm(MURDER ~ PERP_AGE_GROUP, data = df_perp_age_known)
